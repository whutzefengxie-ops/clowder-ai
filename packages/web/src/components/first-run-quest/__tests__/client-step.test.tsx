...
const client = { client: 'codex', provider: 'openai', cli: 'codex', label: 'Codex', installed: true, hasApiKey: false, authenticated: false };
beforeEach(() => {
...
it('does not treat an OAuth account record as CLI authentication', async () => {
  vi.mocked(apiFetch).mockImplementation(async (url) => new Response(JSON.stringify(
    url.includes('available-clients') ? { clients: [client] } : { providers: [{ id: 'codex', authType: 'oauth' }] },
  )));
  await act(async () => { root.render(<ClientStep onSelect={vi.fn()} />); });
...
it('keeps pending until a fresh native credential probe succeeds', async () => {
  ...
  await act(async () => { root.render(<ClientStep savedClients={[{ ...client, authStatus: 'pending' }]} onClientsChange={onChange} onSelect={vi.fn()} />); });
