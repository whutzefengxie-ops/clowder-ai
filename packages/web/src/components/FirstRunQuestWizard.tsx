        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          ...
          {step === 'demo' && (
            <div className="space-y-4 py-4">
              ...
              <div className="flex gap-2"><button type="button" onClick={toggleDemoPause} className="flex-1 rounded-lg border border-[var(--console-border-soft)] py-2.5 text-sm font-semibold text-cafe-secondary">{journey.demoPaused ? '继续' : '暂停'}</button><button type="button" onClick={advanceDemo} disabled={journey.demoPaused || journey.demoScene === 'handoff'} className="flex-1 rounded-lg bg-[var(--semantic-warning)] py-2.5 text-sm font-semibold text-[var(--cafe-surface)] disabled:opacity-50">{journey.demoScene === 'opening' ? '开始演示' : journey.demoScene === 'improved' ? '进入真实配置' : '下一幕'}</button></div>
            </div>
          )}
