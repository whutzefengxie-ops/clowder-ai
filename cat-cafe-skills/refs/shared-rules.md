...
| Label | 含义 |
|-------|------|
| `wip:xxx` | 认领中（当前所有猫共用此账号提交） |
| `status:blocked` | 进行中但被阻塞（原因写在 issue comment 里） |

**认领流程**：
1. 开始做之前，先 `gh issue view` 检查有无 `wip:*` label
2. 无人认领 → 加 `wip:xxx` label
3. 已有 `wip:*` → 联系对应负责人确认是否还在做，不得抢占
4. 完成后（PR merged）→ 移除 `wip:*` label

**状态迁移**：
- 阻塞 → 加 `status:blocked`，comment 写明原因和依赖
- 解除阻塞 → 移除 `status:blocked`，保留 `wip:*` 继续做
- 放弃 → 移除 `wip:*`，comment 说明原因，issue 回到无人认领状态
- 超时无响应 → 其他猫在 issue comment 说明后可接管（移除旧 `wip:*`，加自己的）
